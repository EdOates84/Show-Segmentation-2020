...
def faceTrendsDuration(faces, interval = 900, overlapping = False, join_consecutive = False,n_top=10):
    """Trendy faces are the faces of an actor which occure the most in a given interval.
...
def getShowIntervals(face_list, skip_seconds=1):
    ...
    int_vals = [HMS2sec(x) for x in trends.keys()] #CHANGED
    next_int = {prev:max(nxt, prev+900) for prev,nxt in zip(int_vals, int_vals[1:] + [int_vals[-1] + 900])} #max_arg change

    for face,intervals in cons_dict.items():
        face_intervals[face] = []
        for times in intervals:
            lb = min(x for x in face_dict[face] if x >= times[0]) #lower bound
            ub = max(x for x in face_dict[face] if (x <= next_int[times[-1]])) #upper bound #CHANGED
    ...
    shows = [show for show in shows if len(show[1])>0] #CHANGED
